...
| `dashboard/building.js` + `backend/building.py` | **Building tab** (landing page): six commercial building profiles (office, mall, hospital, hotel, college, data center), operating modes, executive KPIs, floors → zones overview, demand vs expected, zone drill-down with the controller's "why" — `docs/BUILDING_PROFILES.md` |
| `backend/security/` + `dashboard/security.js` + `scripts/security_users.py` | **Security** (Phase 6): development / enforced / production modes, scrypt passwords, opaque bearer sessions, 6 roles with zone-scoped HVAC control, fail-closed route policy, device identity registry with HMAC-signed telemetry, replay / duplicate / spoof detection and quarantine, secure zone control, redacted audit log, rate limits, CORS allowlist, security headers, simulated MQTT adapter with topic ACLs, Security tab. **The default local run is development mode: no enforced auth, plain HTTP, not encrypted** — `docs/SECURITY.md` |
| `backend/scenario.py` + `backend/simhw.py` + `dashboard/scenario.js` | **Seasonal simulation + simulated hardware** (Phase 5): summer / monsoon / winter / transition weather (the Phase-2 weather model) driving the live twin, bounded overrides, envelope and internal-load physics, visible HVAC capacity limits, causal explanation, season comparison; 20 SIMULATED sensors (not real hardware) with noise, drift and faults through the same latest-value pipeline. Heating is NOT MODELLED in the live twin — `docs/SEASONAL_SIMULATION.md`, `docs/HARDWARE_SIMULATION.md` |
| `backend/latest.py` + `backend/telemetry_publish.py` | **Latest-value telemetry** (Phase 4): every current value with unit, source, quality, timestamp and age; the digital twin publishes through the same validated ingest path hardware uses; `/api/latest*`; one central 5-s dashboard poll, Building-tab "Current conditions", telemetry + sensor health, live trend. The twin is the source today — **not** physical telemetry — `DATA_CONTRACTS.md` §13 |
| `backend/comfort.py` + `comfort_events.py` + `comfort_whatif.py` + `dashboard/comfort.js` | **Occupant comfort** (Phase 3): engineering comfort index per zone (thermal / humidity / CO₂ ventilation indicator, profile weights), occupancy relevance, root cause + recommendation, comfort events and discomfort duration, comfort-vs-energy what-if, Comfort tab + drill-down block + occupant-page card — `docs/COMFORT.md` |
| `backend/dataset/` + `scripts/generate_dataset.py` + `dashboard/history.js` | **Historical dataset + History tab** (Phase 2): 6 buildings, 35 zones, 42 days at 5 min generated from the twin physics (seasonal weather → occupancy → HVAC → comfort → energy/demand), RAW/CLEAN sensor layers, anomalies, SQLite store, `/api/history*`, comparisons and export — all labelled SIMULATED history — `docs/DATASET.md` |
