  ...
  function monitorPanel() {
    ...
    function renderKpis() {
      ...
      cards.push(kpiCard({ key: 'hw', label: 'Physical zone (rig · Conference B)', unit: hw && hw.connected && r ? '°C' : '', source: 'hardware',
        ...
        prevText: hw && hw.connected && r ? ('RH ' + (r.rh_pct === null ? '—' : f(r.rh_pct, 1) + ' %') + ' · fan ' + hw.fan + ' · ' + f(hw.stale_s, 1) + ' s ago · ' + esc(hw.node_id)) : (hw && hw.polls ? 'last node stale ' + f(hw.stale_s, 0) + ' s' : 'ESP32 never posted — run scripts.mock_node to rehearse'),
